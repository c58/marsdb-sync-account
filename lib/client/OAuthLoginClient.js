...
class BasicOAuthLoginClient extends BasicLoginClient {
  ...
  login(serviceName) {
    ...
    const serviceUrl = `/auth/oauth/popup/${serviceName}/${credentialToken}`;

    return new Promise((resolve, reject) => {
      this._showPopup(serviceUrl, () => {
        const secret = _credentialSecrets[credentialToken];
        if (!secret) {
          reject(new Error('No secret for given credential token'));
        } else {
          delete _credentialSecrets[credentialToken];
          MarsSync.call('/auth/oauth/secret/login', credentialToken, secret)
  ...
  loginWithToken(serviceName, accessToken) {
    return MarsSync.call('/auth/oauth/token/login', serviceName, accessToken)
...
class AOAuth_Browser extends BasicOAuthLoginClient {
  ...
  _showPopup(url, callback, dimensions) {
    ...
    var popup = this._openCenteredPopup(
      url,
    ...
    var checkPopupOpen = setInterval(() => {
      try {
