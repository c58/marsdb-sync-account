...
// Internals
const TOKEN_EXPIERS_IN = 3 * 30 * 24 * 60 * 60; // 3 months
...
export default class AccountManager extends EventEmitter {
  ...
  authConnection(conn, userId, usedToken) {
    ...
    return users.findOne(userId).then(user => {
      const { validTokens, token } =
        this._filterTokensAndCreate(userId, user.tokens, usedToken);
      return users.update(userId, { $set: { tokens: validTokens } })
        .then(() => { userId, token });
    });
  }
  ...
  _filterTokensAndCreate(userId, tokens = [], usedToken) {
    ...
    const token = jwt.sign({ userId: userId }, this._secretKey,
