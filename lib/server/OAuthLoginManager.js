...
export default class OAuthLoginManager {
  ...
  _handleSecretLogin = (ctx, credentialToken, secret) => {
    ...
    if (savedCred && savedCred.credentialSecret === secret) {
      return this._accManager.loginUser(ctx.connection, savedCred.userId);
    } else {
  ...
  _handleTokenLogin = (ctx, provider, accessToken) => {
    ...
    return new Promise((resolve, reject) => {
      passport.authenticate(provider, (err, user, info) => {
        if (!err && user) {
          resolve(this._accManager.loginUser(ctx.connection, user._id));
        } else {
          reject(err);
        }
      })(reqMock, {});
  ...
  _handleOAuthLogin = (accessToken, refreshToken, profile, done) => {
    ...
    const newUserObj = this._accManager.createUserObject(profile);
